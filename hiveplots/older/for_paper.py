...
for log_deg in [True, False]:
    ...
    phys_H = HivePlot(phys, config_path='config/paper_whole.ini', order_nodes_by='log_degree' if log_deg else 'degree')
    phys_H.draw()
    ...
    H = HivePlot(whole, parent_hiveplot=phys_H, config_path='config/paper_whole.ini')
    H.draw()
